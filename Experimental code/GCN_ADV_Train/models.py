...
class GCN(Model):
    def __init__(self, placeholders, input_dim, attack=None, **kwargs):
        ...
        # if attack is True, placeholders['adj'] feeds in raw adjacent matrix and placeholder['s'] feeds in attack placeholders
        ...
        if self.attack:
            ...
            self.mask = [tf.constant(np.triu(np.ones([self.A[0].shape[0]] * 2, dtype=np.float32), 1))]

            self.C = [1 - 2 * self.A[i] - tf.eye(self.A[i].shape[0], self.A[i].shape[1]) for i in range(self.num_supports)]
            ...
            # here use matrix_band_part to ensure a strictly upper triangular matrix
            self.upper_S_real = [tf.linalg.band_part(self.upper_S_0[i], 0, -1) - tf.linalg.band_part(self.upper_S_0[i], 0, 0) for i in range(self.num_supports)]
            ...
            self.hat_A = [tf.cast(self.modified_A[i] + tf.eye(self.modified_A[i].shape[0], self.modified_A[i].shape[1]), dtype='float32') for i in range(self.num_supports)]
...
class GAT(Model):
    def __init__(self, placeholders, input_dim, attack=None, **kwargs):
        ...
        if self.attack:
            ...
            self.rowsum = tf.reduce_sum(self.hat_A[0], axis=1)
            self.d_sqrt = tf.sqrt(self.rowsum)
            self.d_sqrt_inv = tf.math.reciprocal(self.d_sqrt)
